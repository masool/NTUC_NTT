...
curl -s -X POST \
  ...
  -d '{
	"channelName":"ntuc-channel",
	"channelConfigPath":"../artifacts/channel/ntuc-channel.tx"
}'
```
...
```
curl -s -X POST \
  http://localhost:4000/channels/ntuc-channel/peers \
  -H "authorization: Bearer <put JSON Web Token here>" \
  -H "content-type: application/json" \
  -d '{
	"peers": ["peer0.org1.ntuc.com","peer1.org1.ntuc.com"]
}'
```
### Install chaincode
...
curl -s -X POST \
  ...
  -d '{
	"peers": ["peer0.org1.ntuc.com","peer1.org1.ntuc.com"],
	"chaincodeName":"mycc",
	"chaincodePath":"github.com/ntuc_cc/go",
...
curl -s -X POST \
  ...
  -d '{
	"peers": ["peer0.org1.ntuc.com","peer1.org1.ntuc.com"],
	"chaincodeName":"mycc",
	"chaincodePath":"$PWD/artifacts/src/github.com/ntuc_cc/node",
	"chaincodeType": "node",
	"chaincodeVersion":"v0"
}'
```
...
```
curl -s -X POST \
  http://localhost:4000/channels/ntuc-channel/chaincodes \
  -H "authorization: Bearer <put JSON Web Token here>" \
  -H "content-type: application/json" \
  -d '{
	"chaincodeName":"mycc",
...
```
curl -s -X POST \
  http://localhost:4000/channels/ntuc-channel/chaincodes/mycc \
  -H "authorization: Bearer <put JSON Web Token here>" \
  -H "content-type: application/json" \
  -d '{
	"peers": ["peer0.org1.ntuc.com","peer0.org2.ntuc.com"],
	"fcn":"move",
	"args":["a","b","10"]
}'
```
...
```
curl -s -X GET \
  "http://localhost:4000/channels/ntuc-channel/chaincodes/mycc?peer=peer0.org1.ntuc.com&fcn=query&args=%5B%22a%22%5D" \
  -H "authorization: Bearer <put JSON Web Token here>" \
  -H "content-type: application/json"
```
...
```
curl -s -X GET \
  "http://localhost:4000/channels/ntuc-channel/blocks/1?peer=peer0.org1.ntuc.com" \
  -H "authorization: Bearer <put JSON Web Token here>" \
  -H "content-type: application/json"
```
...
```
curl -s -X GET http://localhost:4000/channels/ntuc-channel/transactions/<put transaction id here>?peer=peer0.org1.ntuc.com \
  -H "authorization: Bearer <put JSON Web Token here>" \
  -H "content-type: application/json"
```
...
```
curl -s -X GET \
  "http://localhost:4000/channels/ntuc-channel?peer=peer0.org1.ntuc.com" \
  -H "authorization: Bearer <put JSON Web Token here>" \
  -H "content-type: application/json"
```
...
```
curl -s -X GET \
  "http://localhost:4000/chaincodes?peer=peer0.org1.ntuc.com&type=installed" \
  -H "authorization: Bearer <put JSON Web Token here>" \
  -H "content-type: application/json"
```
...
```
curl -s -X GET \
  "http://localhost:4000/chaincodes?peer=peer0.org1.ntuc.com&type=instantiated" \
  -H "authorization: Bearer <put JSON Web Token here>" \
  -H "content-type: application/json"
```
...
```
curl -s -X GET \
  "http://localhost:4000/channels?peer=peer0.org1.ntuc.com" \
  -H "authorization: Bearer <put JSON Web Token here>" \
  -H "content-type: application/json"
```
