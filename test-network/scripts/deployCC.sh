...
if [ "$CC_SRC_LANGUAGE" = "go" -o "$CC_SRC_LANGUAGE" = "golang" ] ; then
	...
	CC_SRC_PATH="../chaincode/ntuc/go/"

	echo Vendoring Go dependencies ...
	pushd ../chaincode/ntuc/go
	...
	CC_SRC_PATH="../chaincode/ntuc/javascript/"

elif [ "$CC_SRC_LANGUAGE" = "java" ]; then
	CC_RUNTIME_LANGUAGE=java
	CC_SRC_PATH="../chaincode/ntuc/java/build/install/ntuc"

	echo Compiling Java code ...
	pushd ../chaincode/ntuc/java
	./gradlew installDist
	popd
	...
	CC_RUNTIME_LANGUAGE=node # chaincode runtime language is node.js
	CC_SRC_PATH="../chaincode/ntuc/typescript/"

	echo Compiling TypeScript code into JavaScript ...
	pushd ../chaincode/ntuc/typescript
	npm install
	npm run build
	popd
...
packageChaincode() {
  ...
  setGlobals $PEER $ORG
  set -x
  peer lifecycle chaincode package ntuc.tar.gz --path ${CC_SRC_PATH} --lang ${CC_RUNTIME_LANGUAGE} --label ntuc_${VERSION} >&log.txt
  res=$?
  set +x
  cat log.txt
...
installChaincode() {
  ...
  setGlobals $PEER $ORG
  set -x
  peer lifecycle chaincode install ntuc.tar.gz >&log.txt
  res=$?
  set +x
  cat log.txt
...
queryInstalled() {
  ...
  set +x
  cat log.txt
	PACKAGE_ID=$(sed -n "/ntuc_${VERSION}/{s/^Package ID: //; s/, Label:.*$//; p;}" log.txt)
...
approveForMyOrg() {
  ...
  setGlobals $PEER $ORG
  set -x
  peer lifecycle chaincode approveformyorg -o localhost:7050 --ordererTLSHostnameOverride orderer.ntuc.com --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name ntuc --version ${VERSION} --init-required --package-id ${PACKAGE_ID} --sequence ${VERSION} >&log.txt
  set +x
  cat log.txt
...
checkCommitReadiness() {
  ...
  # we either get a successful response, or reach MAX RETRY
    ...
    set -x
    peer lifecycle chaincode checkcommitreadiness --channelID $CHANNEL_NAME --name ntuc --version ${VERSION} --sequence ${VERSION} --output json --init-required >&log.txt
    res=$?
    set +x
    let rc=0
    for var in "$@"
...
commitChaincodeDefinition() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer.ntuc.com --channelID $CHANNEL_NAME --name ntuc $PEER_CONN_PARMS --version ${VERSION} --sequence ${VERSION} --init-required >&log.txt
    res=$?
    set +x
  else
    set -x
    peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer.ntuc.com --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name ntuc $PEER_CONN_PARMS --version ${VERSION} --sequence ${VERSION} --init-required >&log.txt
    res=$?
    set +x
  fi
  cat log.txt
...
queryCommitted() {
  ...
  # we either get a successful response, or reach MAX RETRY
    ...
    set -x
    peer lifecycle chaincode querycommitted --channelID $CHANNEL_NAME --name ntuc >&log.txt
    res=$?
    set +x
...
chaincodeInvokeInit() {
  ...
  # it using the "-o" option
  set -x
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.ntuc.com --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n ntuc $PEER_CONN_PARMS --isInit -c '{"function":"init","Args":[]}' >&log.txt
  res=$?
  set +x
  cat log.txt
...
#     set -x
#     peer chaincode query -C $CHANNEL_NAME -n ntuc -c '{"Args":["queryAllCars"]}' >&log.txt
#     res=$?
#     set +x
# 		let rc=$res
# 		COUNTER=$(expr $COUNTER + 1)
...
## Install chaincode on peer0.tradeunion and peer0.tradeassociation
echo "Installing chaincode on peer0.tradeunion..."
installChaincode 0 1
echo "Install chaincode on peer1.tradeunion..."
installChaincode 1 1
echo "Install chaincode on peer0.tradeassociation..."
installChaincode 0 2
echo "Install chaincode on peer1.tradeassociation..."
installChaincode 1 2
echo "Install chaincode on peer0.socialenterprise..."
installChaincode 0 3
echo "Install chaincode on peer1.socialenterprise..."
...
## approve the definition for tradeunion
approveForMyOrg 0 1

## check whether the chaincode definition is ready to be committed
## expect tradeunion to have approved and tradeassociation not to
...
## now approve also for tradeassociation
approveForMyOrg 0 2
...
# # # Query chaincode on peer0.tradeunion
# # echo "Querying chaincode on peer0.tradeunion..."
# # chaincodeQuery 1
echo
echo "========= All GOOD, ntuc Business Network execution completed =========== "
echo

echo
echo " +-+-+-+-+-+-+-+-+-+-+-+-+ +-+-+-+";
echo " |n|t|u|c|-|n|e|t|w|o|r|k| |e|n|d|";
echo " +-+-+-+-+-+-+-+-+-+-+-+-+ +-+-+-+";
echo
